...
// StreamSender is the interface satisfied by gRPC server streams (used to
// decouple Engine from the generated pb stream type so it stays testable).
type StreamSender interface {
	Send(chunk *pb.WorkflowStreamChunk) error
}

// ExecuteStream executes a workflow and streams node lifecycle events to the
// caller. It reuses the same setup logic as Execute but wires
// executor.SetStreamEventFunc so each node_started / node_completed / node_error
// event is forwarded in real time. A final chunk (type "final") carries the
// complete ExecuteWorkflowResponse once the DAG finishes.
func (e *Engine) ExecuteStream(ctx context.Context, workflowID string, input string, tenantID string, timeoutMs int64, stream StreamSender) error {
	wfModel, err := e.workflowRepo.Get(ctx, workflowID)
	if err != nil {
	...
	if saveErr := e.executionRepo.Save(ctx, execRecord); saveErr != nil {
		return fmt.Errorf("save execution record: %w", saveErr)
	}
	...
	// Wire streaming: forward each node lifecycle event to the gRPC stream.
	executor.SetStreamEventFunc(func(eventType, nodeID, nodeName, nodeType, output string, err error) {
		chunk := &pb.WorkflowStreamChunk{
			Type:     eventType,
			NodeId:   nodeID,
			NodeName: nodeName,
			NodeType: nodeType,
	...
	if err != nil {
		...
		execRecord.Error = err.Error()
		_ = stream.Send(&pb.WorkflowStreamChunk{Type: "error", Error: err.Error()})
	} else {
		execRecord.Status = string(StatusCompleted)
	}
	...
	finalResp := &pb.ExecuteWorkflowResponse{
		WorkflowId:  workflowID,
		ExecutionId: execID,
	}
	if result != nil {
		finalResp.FinalOutput = result.FinalOutput
		for _, nr := range result.Nodes {
			finalResp.Nodes = append(finalResp.Nodes, &pb.WorkflowNodeResult{
				NodeId:     nr.NodeID,
	...
	_ = stream.Send(&pb.WorkflowStreamChunk{Type: "final", FinalResult: finalResp})
	return nil
}
