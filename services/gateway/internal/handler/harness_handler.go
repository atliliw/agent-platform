...
// ExecuteWorkflowStream executes a workflow with real-time SSE streaming of node events.
// Each node_started / node_completed / node_error is forwarded as an SSE event;
// a final event carries the complete execution result.
func (h *RealHarnessHandler) ExecuteWorkflowStream(c *gin.Context) {
	if h.client == nil {
		c.JSON(200, gin.H{"code": -1, "message": "harness service not available"})
		return
	}
	...
	stream, err := h.client.ExecuteWorkflowStream(ctx, &pb.ExecuteWorkflowRequest{
		Id:             id,
		Input:          req.Input,
		TimeoutSeconds: req.TimeoutSeconds,
	})
	...
	for {
		...
		if chunk.FinalResult != nil {
			data["final_result"] = gin.H{
				"workflow_id":  chunk.FinalResult.WorkflowId,
				"execution_id": chunk.FinalResult.ExecutionId,
				"final_output": chunk.FinalResult.FinalOutput,
				"status":       chunk.FinalResult.Status,
				"error":        chunk.FinalResult.Error,
			}
		}
