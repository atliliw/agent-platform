...
func (s *ChatService) chatStreamWithMultiAgent(ctx context.Context, req *pb.ChatRequest, stream pb.ChatService_ChatStreamServer) error {
	...
	for {
		...
		case "approval":
			// chunk.Content carries JSON {request_id, tool_name, reason} from
			// the engine (serialized in agent-service). Pass through unchanged
			// so the frontend can parse request_id and submit a decision via
			// /api/v2/harness/approval/approve|reject. The agent is blocked on
			// WaitForApproval and resumes once a decision is submitted.
		case "final":
