...
func (p *BrowserPool) createNewBrowser(ctx context.Context) (*Browser, error) {
	...
	if obscuraURL != "" {
		...
		allocCtx, _ := chromedp.NewRemoteAllocator(ctx, obscuraURL, chromedp.NoModifyURL) // NoModifyURL: 否则 chromedp 会 fetch /json/version，obscura 在里面 advertise 127.0.0.1，跨容器连不上
		browserCtx, cancel := chromedp.NewContext(allocCtx)

		// Obscura 不触发 Page.frameStoppedLoading，chromedp.Navigate 会永久挂起。
		// 用一次轻量 evaluate 确认连接成功，而不是 Navigate+WaitReady。
		var ua string
