...
const (
	...
	ActionEditorInput  ActionType = "editor_input" // Markdown 编辑器输入
	ActionEditorTitle  ActionType = "editor_title" // 标题输入
	ActionPublish      ActionType = "publish"      // 发布按钮
		ActionExecuteJS    ActionType = "execute_js"  // 执行 JavaScript
)
...
type Action struct {
	...
	JavaScript string    `json:"javascript,omitempty"` // JavaScript 代码
...
func (a *Agent) RunWithDomain(ctx context.Context, task string, domain string) (*Result, error) {
	...
	for step := 0; step < a.options.MaxSteps; step++ {
		...
		fmt.Printf("DEBUG: Elements count: %d\n", len(state.Elements))
		for i, el := range state.Elements {
			if i < 20 || strings.Contains(el.Text, "标题") || strings.Contains(el.Text, "输入") || strings.Contains(el.Text, "编辑") || el.Tag == "input" || el.Tag == "textarea" {
				fmt.Printf("  [%d] %s: %q (selector: %s)\n", el.Index, el.Tag, el.Text, el.Selector)
			}
		}
		...
		fmt.Printf("DEBUG: ExecuteAction result: %s, err: %v, action.Type: %s, ActionInput: %s\n", result, err, action.Type, ActionInput)
		...
		// 如果是输入操作，检查是否应该继续
		// 不再自动返回，让 LLM 决定是否继续
		if action.Type == ActionInput && err == nil {
			fmt.Printf("DEBUG: Input completed, continuing for next step\n")
		}
		fmt.Printf("DEBUG: Not ActionInput, continuing loop. action.Type=%s\n", action.Type)
	}
...
func buildUserPrompt(task string, state *PageState, lastResult string) string {
	...
	if lastResult != "" {
		...
		sb.WriteString("\n\n**如果上一步已成功完成输入操作，请立即调用 done 返回结果。**\n")
	}

	sb.WriteString("\n请决定下一步动作，以 JSON 格式回复。\n")
	sb.WriteString("**重要：每个动作只能执行一次，不要重复。输入完成后立即调用 done。**\n")
