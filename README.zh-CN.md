...
平台采用 Go + gRPC 微服务架构，Vue 3 前端，通过 Docker Compose 部署，默认使用阿里云 DashScope（通义千问）作为 LLM 提供方。
...
- `protoc` + `protoc-gen-go` + `protoc-gen-go-grpc`（用于生成 protobuf）
- Node.js ≥ 18（前端开发）

### 步骤

```bash
# 1. 配置环境变量（你的 DashScope API Key）
cp .env.example .env
...
API Key 通过 `OPENAI_API_KEY` 环境变量注入，**不**硬编码在 `config.yaml` 中。在 <https://dashscope.console.aliyun.com/> 获取 DashScope Key。
...
每个服务读取 `config.yaml`（以只读方式挂载进容器）。敏感值通过环境变量注入，由 Docker Compose 从 `.env` 加载：

| 变量 | 用途 |
|------|------|
| `OPENAI_API_KEY` | LLM API Key（DashScope）。**必填** |
| `LLM_PROVIDER` | 覆盖 `llm.provider` |
| `LLM_MODEL` | 覆盖 `llm.model` |
| `QDRANT_URL` | 覆盖 Qdrant 地址 |
| `MONGODB_URL` | 覆盖 MongoDB 地址 |
| `REDIS_URL` | 覆盖 Redis 地址 |
